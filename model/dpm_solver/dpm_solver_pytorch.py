...
def model_wrapper(model, noise_schedule=None, is_cond_classifier=False, classifier_fn=None, classifier_scale=1., time_input_type='0', total_N=1000, model_kwargs={}):
    ...
    def model_fn(x, t_continuous):
        ...
        if is_cond_classifier:
            ...
            t_discrete = get_model_input_time(t_continuous)
            score = model(x, t_discrete, **model_kwargs)
            std = noise_schedule.marginal_std(t_continuous)
            noise = -score * std[:, None, None, None]
            return noise
...
class DPM_Solver:
    ...
    def dpm_solver_first_update(self, x, s, t, condition_x, return_noise=False):
        """
        ...
        if condition_x is not None:
            noise_s = self.model_fn(torch.cat([condition_x, x], dim=1), s)
        else:
    ...
    def dpm_solver_second_update(self, x, s, t, condition_x, r1=0.5, noise_s=None, return_noise=False):
        """
        ...
        if condition_x is not None:
            noise_s1 = self.model_fn(torch.cat([condition_x, x_s1], dim=1), s1)
        else:
    ...
    def dpm_solver_third_update(self, x, s, t, condition_x, r1=1./3., r2=2./3., noise_s=None, noise_s1=None, noise_s2=None):
        """
        ...
        if noise_s2 is None:
            ...
            if condition_x is not None:
                noise_s2 = self.model_fn(torch.cat([condition_x, x_s2], dim=1), s2)
            else:
    ...
    def dpm_solver_update(self, x, s, t, order, condition_x):
        """
        ...
        """
        if order == 1:
            return self.dpm_solver_first_update(x, s, t, condition_x)
        elif order == 2:
            return self.dpm_solver_second_update(x, s, t, condition_x)
        elif order == 3:
            return self.dpm_solver_third_update(x, s, t, condition_x)
        else:
            raise ValueError("Solver order must be 1 or 2 or 3, got {}".format(order))

    def dpm_solver_adaptive(self, x, order, condition_x, t_T, t_0, h_init=0.05, atol=0.0078, rtol=0.05, theta=0.9, t_err=1e-5):
        """
        ...
        if order == 2:
            r1 = 0.5
            lower_update = lambda x, s, t: self.dpm_solver_first_update(x, s, t, condition_x, return_noise=True)
            higher_update = lambda x, s, t, **kwargs: self.dpm_solver_second_update(x, s, t, condition_x, r1=r1, **kwargs)
        elif order == 3:
            r1, r2 = 1. / 3., 2. / 3.
            lower_update = lambda x, s, t: self.dpm_solver_second_update(x, s, t, condition_x, r1=r1, return_noise=True)
            higher_update = lambda x, s, t, **kwargs: self.dpm_solver_third_update(x, s, t, condition_x, r1=r1, r2=r2, **kwargs)
        else:
    ...
    def sample(self, x, y, Afun, Ainv, mask, coeff, steps=10, eps=1e-4, T=None, order=3, skip_type='logSNR',
        adaptive_step_size=False, fast_version=True, atol=0.0078, rtol=0.05, condition_x = None
    ):
        """
        ...
        **We recommend DPM-Solver-fast for both fast sampling in few steps (<=20) and fast convergence in many steps (50 to 100).**
        ...
        if adaptive_step_size:
            with torch.no_grad():
                x = self.dpm_solver_adaptive(x, order=order, condition_x=condition_x, t_T=t_T, t_0=t_0, atol=atol, rtol=rtol)
        else:
            if fast_version:
            ...
            with torch.no_grad():
                for i, order in enumerate(orders):
                    ...
                    x = self.merged_known(x, y, Afun, Ainv, mask, coeff, vec_s)
                    x = self.dpm_solver_update(x, vec_s, vec_t, order, condition_x)
        return x

    def merged_known(self, x, y, Afun, Ainv, mask, coeff, t):
        known_sino = self.get_known_sino(x, y, Afun, t)
        x_sino = Afun(x)
        merged_sino = x_sino * (1 - coeff) + known_sino * coeff
        merged_sino = merged_sino * mask + x_sino * (1-mask)
        known_x = Ainv(merged_sino, x)
        return known_x

    def get_known_sino(self, x, y, Afun, t):
        ns = self.noise_schedule
        log_mean_coeff = ns.marginal_log_mean_coeff(t[:y.shape[0]])
        mean = torch.exp(log_mean_coeff[:, None, None, None]) * y
        std = ns.marginal_std(t[:y.shape[0]])
